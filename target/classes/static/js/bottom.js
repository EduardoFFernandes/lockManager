...
function initI18nTelInput() {
    ...
    if ($('.tdPhone')[0]) {
        var arrayInputs = $('.tdPhone');
        arrayInputs.toArray().forEach(input => {
...
function handleI18nTelTd(inputEl, iti) {
    var countryMask;
...
function handleCheckAssociationsResponse(response) {
    if (response) {
        ...
        modalDeleteWarning.find(':submit').attr('disabled', true);
        modalDeleteWarning.find(':submit').css('cursor', 'default');
    }
}
